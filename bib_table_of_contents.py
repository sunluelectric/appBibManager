...
class BibTableOfContents:
    ...
    This class provides functionality to create, display, and manipulate the table of contents
    for a document or publication. It stores the section indexes, section layers, and section names
    in a dictionary format. The table of contents can be created from a multi-dimensional list
    or a list of sections with 4 spaces used to describe the subsection layer index.

    The class also includes methods to display the table of contents in the console,
    return the table of contents in a list with printout format, and retrieve all section indexes.
    ...
    def create_tocs_from_space_list(self, lst_text: list):
        ...
        for iter_item in lst_text:
            if 1 <= (len(iter_item) - len(iter_item.lstrip(' '))) // 4 + 1 <= 8:
                self.int_section_layer = (
                    len(iter_item) - len(iter_item.lstrip(' '))) // 4 + 1
            else:
        ...
        if self.dict_tocs:
            ...
            for iter_item in lst_tocs_keys:
                ...
                str_print = hex(iter_item)[-8:] + "    " * \
                    int_layer_of_item + self.dict_tocs[iter_item]
                print(str_print)
        else:
    ...
    def __add_section(self, str_item_name: str):
        ...
        if 1 <= int_layer_num_of_section + 1 <= 15:
            self.hex_section_index = self.hex_section_index + \
                16**(8-self.int_section_layer)
            self.hex_section_index = \
                self.hex_section_index - \
                (self.hex_section_index % 16**(8-self.int_section_layer))
        else:
